...
export class FacebookScraper implements Scraper {
  ...
  private async loadHtml(url: string, proxyUrl?: string): Promise<string> {
    const res = await this.http.get(url, { referer: "https://www.facebook.com/", proxyUrl });
  ...
  async profile(url: string, options?: ScrapeOptions): Promise<ScrapeResult<NormalizedProfile>> {
    const html = await this.loadHtml(url, options?.proxyUrl);
